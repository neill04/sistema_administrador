            ...
            <div class="modal-body">
                <form id="ofertaForm">
                    ...
                    @foreach (['Idiomas', 'Experiencia Laboral', 'Funciones', 'Conocimientos', 'Beneficios', 'Competencias'] as $tipo)
                        <div class="mb-3">
                            <label>{{ $tipo }}</label>
                            <div id="contenedor-{{ Str::slug($tipo) }}">
                                <input type="text" name="atributos[{{ Str::slug($tipo) }}][]" class="form-control mb-2" placeholder="Agregar {{ strtolower($tipo) }}">
                            </div>
                            <button type="button" class="btn btn-primary btn-sm" onclick="agregarCampo('{{ Str::slug($tipo) }}')">+</button>
                        </div>
                    @endforeach
                    </div>
